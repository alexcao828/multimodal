...
def train_model(model, optimizer, num_epochs=100):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val', 'test']:
            ...
            if phase == 'val':
                allActProbs = np.zeros((np.shape(val_labels)[0], T, 2))
                allClassProbs = np.zeros((np.shape(val_labels)[0], T, 24))
                start = 0
            ...
            for h_batch, a_batch, i_batch, c_batch, b_batch, y_batch in dataloaders_dict[phase]:
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    class_p, AA, act_p2 = model(h_batch, a_batch, i_batch, c_batch, b_batch)
                    ...
                    if phase == 'val':
                        stop = start + np.shape(act_p2.cpu().detach().numpy())[0]
                        allActProbs[start:stop, :, :] = act_p2.cpu().detach().numpy()
                        allClassProbs[start:stop, :, :] = class_p.cpu().detach().numpy()
                        start = stop
            ...
            if phase == 'train':
                ...
                np.save('allActProbs'+str(epoch)+'.npy', allActProbs)
                np.save('allClassProbs'+str(epoch)+'.npy', allClassProbs)
            elif phase == 'test':
